...
export async function carregarDadosPrecificacaoComCliente(
  ...
  return {
    pet: {
      ...
      nome: texto(pet.nome),
      especie: texto(pet.especie) as PetMotor['especie'],
      racaId: texto(pet.raca_id),
      racaNome: texto(raca?.nome),
      sexo: texto(pet.sexo) as PetMotor['sexo'],
      porte: texto(pet.porte) as PetMotor['porte'],
      pelagem: texto(pet.pelagem) as PetMotor['pelagem'],
      peso: nuloOuNumero(pet.peso),
      temperamento: texto(pet.temperamento) as PetMotor['temperamento'],
    },
    dados: {
