...
await db.exec(readFileSync('banco/039_pets_incompletos_importacao.sql','utf8'))
...
await test('aplicar 035–039 preserva dados operacionais e sequences CLI/PET',async()=>{assert.deepEqual((await db.query(snapshotSql)).rows[0].estado,antesMigration)})
...
await test('pet parcial é promovido e NULL nunca usa defaults',async()=>{const d=doc('pet-parcial');for(const k of ['especie','sexo','porte','pelagem','temperamento','raca_id','castrado'])d.pets[0].resolvido[k]=null;const s=await rpc('importacao_salvar_lote',[d]);assert.equal(s.status,'pronto');assert.deepEqual(s.pets[0].erros,[]);const p=await rpc('importacao_promover_lote',[s.id,s.revisao,true]);assert.equal(p.status,'concluido');const criado=(await db.query("select especie,raca_id,sexo,porte,pelagem,temperamento,castrado from public.pets where id=$1",[p.pets[0].internal_id])).rows[0];for(const k of ['especie','raca_id','sexo','porte','pelagem','temperamento','castrado'])assert.equal(criado[k],null)})
await test('resoluções anteriores e decisão de ignorar sobrevivem à 039 e à reabertura',async()=>{const d=doc('resolucoes-preservadas');d.pets[0].original={...d.pets[0].original,tamanho:'Micro',comportamento:'manso'};d.pets[0].resolvido={...d.pets[0].resolvido,porte:'mini',temperamento:'calmo'};const ignorado={...linha('ignorado',{...perfilPet,porte:null,temperamento:null},true),decisao_operador:'ignorar'};d.pets.push(ignorado);const s=await rpc('importacao_salvar_lote',[d]);const r=await rpc('importacao_obter_lote',[s.id]);const preservado=r.pets.find(p=>p.external_id==='p');assert.equal(preservado.original.tamanho,'Micro');assert.equal(preservado.original.comportamento,'manso');assert.equal(preservado.resolvido.porte,'mini');assert.equal(preservado.resolvido.temperamento,'calmo');assert.equal(r.pets.find(p=>p.external_id==='ignorado').decisao_operador,'ignorar')})
...
await test('perfil parcial não exige retomada nem recria cliente',async()=>{const d=doc('parcial-direto');d.pets[0].resolvido.temperamento=null;let s=await rpc('importacao_salvar_lote',[d]);s=await rpc('importacao_promover_lote',[s.id,s.revisao,true]);assert.equal(s.status,'concluido');assert.equal(s.resultado.clientes_criados,1);assert.equal(s.resultado.pets_criados,1)})
...
if(process.argv[4]&&process.argv[5])await test('CSVs reais preservam preview, avisos, decisões e resoluções após salvar/reabrir',async()=>{
 ...
 assert.equal(resumoAntes.clientes,329);assert.equal(resumoAntes.pets,383);assert.equal(resumoAntes.vinculos,383);assert.equal(resumoAntes.clientesPendentes,1);assert.equal(resumoAntes.petsPendentes,2);assert.equal(resumoAntes.racasPendentes,297);assert.equal(resumoAntes.duplicidades,4)
...
await test('consulta pós-039 executa integralmente em transação somente leitura',async()=>{await db.exec('reset role; begin; set transaction read only;');const sql=readFileSync('banco/admin/go_live/validar_039_pos_aplicacao.sql','utf8');const consulta=sql.slice(sql.indexOf('with verificacoes')).replace(/rollback;\s*$/i,'');const resultado=await db.query(consulta);assert(resultado.rows.length>=8);assert.deepEqual(resultado.rows.filter(r=>!r.ok),[]);await db.exec('rollback')})
await db.close()
...
await test('falha no meio da 039 reverte nullability, defaults e RPCs',async()=>{
 const isolado=new PGlite();try{await isolado.exec(schemaFixture);await isolado.exec(readFileSync('banco/035_seguranca_clientes_pets_campos_cadastro.sql','utf8'));await isolado.exec(readFileSync('banco/036_importacao_clientes_pets_staging.sql','utf8'));await isolado.exec(readFileSync('banco/037_preservar_duplicidades_importacao_staging.sql','utf8'));await isolado.exec(readFileSync('banco/038_resolucao_racas_staging_em_lote.sql','utf8'))
 const antes=(await isolado.query("select attnotnull,pg_get_expr(d.adbin,d.adrelid) as padrao from pg_attribute a left join pg_attrdef d on d.adrelid=a.attrelid and d.adnum=a.attnum where a.attrelid='public.pets'::regclass and a.attname='pelagem'")).rows[0];const sql=readFileSync('banco/039_pets_incompletos_importacao.sql','utf8').replace('alter table public.pets validate constraint pets_raca_exige_especie_check;','select 1/0;\nalter table public.pets validate constraint pets_raca_exige_especie_check;');await assert.rejects(isolado.exec(sql));await isolado.exec('rollback');const depois=(await isolado.query("select attnotnull,pg_get_expr(d.adbin,d.adrelid) as padrao from pg_attribute a left join pg_attrdef d on d.adrelid=a.attrelid and d.adnum=a.attnum where a.attrelid='public.pets'::regclass and a.attname='pelagem'")).rows[0];assert.deepEqual(depois,antes);assert.equal((await isolado.query("select count(*)::int n from pg_constraint where conrelid='public.pets'::regclass and conname='pets_raca_exige_especie_check'")).rows[0].n,0)
 }finally{await isolado.close()}
})
