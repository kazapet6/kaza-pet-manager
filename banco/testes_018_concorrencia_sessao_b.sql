-- RPC 018 - CONCORRENCIA - SESSAO B
-- ESTE ARQUIVO E A SESSAO B.
-- Abra em outro SQL Editor. Inicie este arquivo durante a primeira espera
-- de 15 segundos da Sessao A; os cenarios seguintes se sincronizam por locks.
...
-- Esta sessao disputa os locks mantidos pela Sessao A e nao dorme.
begin;
select 'FUNCIONARIO_B' as teste,
  public.confirmar_agendamento_transacional(
    pg_temp.plano_concorrente_rpc018('FUNCIONARIO', 'B')
  ) as resposta;
select 'SESSAO_B_FUNCIONARIO_CONCLUIDA' as estado;
commit;

begin;
select 'EQUIPAMENTO_B' as teste,
  public.confirmar_agendamento_transacional(
    pg_temp.plano_concorrente_rpc018('EQUIPAMENTO', 'B')
  ) as resposta;
select 'SESSAO_B_EQUIPAMENTO_CONCLUIDA' as estado;
commit;

begin;
select 'COMPATIVEL_B' as teste,
  public.confirmar_agendamento_transacional(
    pg_temp.plano_concorrente_rpc018('COMPATIVEL', 'B')
  ) as resposta;
select 'SESSAO_B_COMPATIVEL_CONCLUIDA' as estado;
commit;
