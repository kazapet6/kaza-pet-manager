-- RPC 018 - EQUIPAMENTO - SESSAO B
-- Autossuficiente: execute o arquivo inteiro uma unica vez com Ctrl+A -> Run.
...
create temporary table rpc018_resultado_sessao (
  teste text not null,
...
insert into rpc018_resultado_sessao (teste, resposta_json)
select 'EQUIPAMENTO_B',
  public.confirmar_agendamento_transacional(
    pg_temp.plano_concorrente_rpc018('EQUIPAMENTO', 'B')
  );

commit;

-- Ultimo resultado do script: copie esta linha antes de qualquer limpeza.
select
  resultado.teste,
  resultado.resposta_json ->> 'status' as status,
  resultado.resposta_json ->> 'codigo' as codigo,
  resultado.resposta_json ->> 'grupoAgendamentoId'
    as grupo_agendamento_id,
  resultado.resposta_json ->> 'atendimentoId' as atendimento_id,
  (resultado.resposta_json ->> 'versaoOcupacao')::bigint
    as versao_ocupacao,
  resultado.resposta_json,
  'SESSAO_B_FINALIZADA' as marcador_final
from rpc018_resultado_sessao resultado;
