...
async function rpc(nome:string,args:Record<string,unknown>):Promise<Lote> {
  ...
  if(error) throw new Error(error.code==='PGRST202'?'Importação ainda não instalada no banco. A Migration 036 precisa ser aplicada em etapa autorizada.':'Operação não concluída. Reabra o lote para conferir sua revisão; nenhuma conclusão foi presumida.')
  return data as Lote
}
