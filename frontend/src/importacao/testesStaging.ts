...
t('cliente sem telefone e pets incompletos ficam pendentes',()=>{const l=analisarArquivos(c.replace('11999999999',''),p.replace(';Macho;Pequeno;Curto;calmo;f',';;;;;'),racas,'origem',[]);assert.equal(resumo(l).clientesPendentes,1);for(const v of ['sexo','porte','pelagem','temperamento','castrado'])assert(l.pets[0].erros.includes(v+' pendente'))})
