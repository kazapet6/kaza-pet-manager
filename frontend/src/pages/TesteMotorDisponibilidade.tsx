...
function ResumoPet({ pet, pendencias }: { pet: Pet; pendencias: string[] }) {
  const campos:string[][] = [['Nome', pet.nome], ['Espécie', pet.especie === 'cao' ? 'Cão' : pet.especie === 'gato' ? 'Gato' : 'Não informado'], ['Raça', pet.racaNome ?? 'Não informado'], ['Sexo', pet.sexo === 'macho' ? 'Macho' : pet.sexo === 'femea' ? 'Fêmea' : 'Não informado'], ['Porte', pet.porte ?? 'Não informado'], ['Peso', pet.peso === null ? 'Não informado' : `${pet.peso.toLocaleString('pt-BR')} kg`], ['Pelagem', pet.pelagem ?? 'Não informado'], ['Temperamento', pet.temperamento ?? 'Não informado']]
  return <div className="teste-motor-pet"><h3>Dados usados pelo motor</h3><dl>{campos.map(([nome, valor]) => <div key={nome}><dt>{nome}</dt><dd>{valor}</dd></div>)}</dl>{pendencias.length > 0 && <p className="teste-motor-aviso">Cadastro incompleto: o Motor solicitará apenas o que for necessário para os serviços escolhidos.</p>}</div>
}
...
function validarDadosPet(pet: Pet):string[] { return ([['nome', pet.nome], ['espécie', pet.especie], ['raça', pet.racaId], ['sexo', pet.sexo], ['porte', pet.porte], ['pelagem', pet.pelagem], ['temperamento', pet.temperamento]] as const).filter(([, valor]) => !valor).map(([nome]) => nome) }
...
function rotuloEstado(estado: ResultadoDisponibilidade['estado']) { return { AGENDA_NAO_CONFIGURADA: 'Agenda ainda não configurada', LOJA_FECHADA: 'Estabelecimento fechado', CADASTRO_PET_INCOMPLETO:'Complete os dados necessários do pet', PET_INELEGIVEL: 'Serviço incompatível com o Pet', SERVICO_INVALIDO: 'Configuração do serviço inválida', SEM_DISPONIBILIDADE: 'Nenhuma disponibilidade', OK: 'Disponível' }[estado] }
